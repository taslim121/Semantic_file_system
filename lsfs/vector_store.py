...
class VectorStore:
    ...
    def index_directory(self, directory: str) -> Dict[str, int]:
        ...
        file_iter = iter_files(
            directory, self.config.exclude_dirs, self.config.skip_hidden
        )

        with tqdm(desc="Indexing files") as pbar:
    ...
    def search(
        ...
        try:
            ...
            where_filter = {}
            if file_type:
                where_filter["extension"] = file_type.lower()
            ...
            # Search in ChromaDB (chunk-level)
